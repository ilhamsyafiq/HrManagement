            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-base font-semibold text-gray-900">Edit User Information</h3>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-5">
                        ...
                        <div id="supervisor_container" style="display: none;">
                            ...
                            <select name="supervisor_id" id="supervisor_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm text-sm">
                                ...
                                @foreach($supervisors as $supervisor)
                                    <option value="{{ $supervisor->id }}" {{ old('supervisor_id', $user->supervisor_id) == $supervisor->id ? 'selected' : '' }}>{{ $supervisor->name }} ({{ $supervisor->department->name ?? 'No Dept' }})</option>
                                @endforeach
                            </select>
                        ...
                        <input type="hidden" name="is_intern" id="is_intern" value="{{ old('is_intern', $user->is_intern ? 1 : 0) }}">

                        {{-- Intern-specific fields --}}
                        <div id="intern-fields" class="{{ old('is_intern', $user->is_intern) ? '' : 'hidden' }}">
                            <div class="bg-blue-50/70 rounded-xl border border-blue-100 p-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input id="internship_start_date" name="internship_start_date" type="date" class="mt-1 block w-full" :value="old('internship_start_date', $user->internship_start_date ? $user->internship_start_date->format('Y-m-d') : '')" />
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="internship_end_date" name="internship_end_date" type="date" class="mt-1 block w-full" :value="old('internship_end_date', $user->internship_end_date ? $user->internship_end_date->format('Y-m-d') : '')" />
