                                        ...
                                        @if(auth()->user()->isSuperAdmin() || auth()->user()->isAdmin())
                                            <div class="flex items-center gap-3">
                                                ...
                                                @if(!$user->isSuperAdmin())
                                                    <form method="POST" action="{{ route('admin.users.delete', $user->id) }}" onsubmit="return confirm('Are you sure you want to delete {{ addslashes($user->name) }}? This action cannot be undone.')">
                                                        @csrf
                                                        @method('DELETE')
