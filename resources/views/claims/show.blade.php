                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        @if($item->receipt_path)
                                            ...
                                            @if(in_array($ext, ['jpg', 'jpeg', 'png']))
                                                <a href="{{ route('claims.receipt', $item->id) }}" target="_blank" class="group inline-flex items-center gap-2">
                                                    <img src="{{ route('claims.receipt', $item->id) }}" alt="Receipt" class="w-10 h-10 object-cover rounded-lg border border-gray-200 group-hover:border-blue-400 transition-colors duration-150">
                                                    <span class="text-blue-600 hover:text-blue-800 font-medium text-xs transition-colors duration-150">View</span>
                                                </a>
                                            @else
                                                <a href="{{ route('claims.receipt', $item->id) }}" target="_blank" class="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-medium transition-colors duration-150">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
