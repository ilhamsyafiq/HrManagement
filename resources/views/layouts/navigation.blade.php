        ...
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-4 sm:-my-px sm:ml-10 sm:flex items-center">
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->isAdmin())
                        ...
                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                            <button @click="open = !open" class="inline-flex items-center h-16 px-1 pt-1 border-b-2 text-sm font-medium leading-5 transition duration-150 ease-in-out focus:outline-none {{ request()->routeIs('admin.users.*') || request()->routeIs('admin.attendances') || request()->routeIs('admin.leaves') || request()->routeIs('leave.approvals') ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                {{ __('People') }}
                        ...
                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                            <button @click="open = !open" class="inline-flex items-center h-16 px-1 pt-1 border-b-2 text-sm font-medium leading-5 transition duration-150 ease-in-out focus:outline-none {{ request()->routeIs('claims.*') || request()->routeIs('announcements.*') || request()->routeIs('calendar.*') || request()->routeIs('holidays.*') || request()->routeIs('admin.departments') ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                {{ __('Organization') }}
                                <svg class="ml-1 h-4 w-4 transition-transform" :class="{ 'rotate-180': open }" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                            </button>
                            <div x-show="open" x-transition class="absolute z-50 mt-2 w-48 rounded-xl bg-white dark:bg-gray-700 shadow-lg border border-gray-100 dark:border-gray-600 py-1" style="display: none;">
                                <a href="{{ route('admin.departments') }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">{{ __('Departments') }}</a>
                    ...
                    @else
                        ...
                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                            ...
                            <div x-show="open" x-transition class="absolute z-50 mt-2 w-48 rounded-xl bg-white dark:bg-gray-700 shadow-lg border border-gray-100 dark:border-gray-600 py-1" style="display: none;">
                                ...
                                @if(auth()->user()->department_id || auth()->user()->isSupervisor())
                                <a href="{{ route('attendance.team') }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">{{ __('Team Attendance') }}</a>
                                @endif
                        ...
                        {{-- Intern Reports (supervisor): top-level so pending reviews aren't missed --}}
                        @if(auth()->user()->isSupervisor())
                            @php
                                $pendingReports = \App\Models\Document::where('type', 'Internship Report')
                                    ->whereIn('user_id', auth()->user()->subordinates()->where('is_intern', true)->pluck('id'))
                                    ->where('status', 'pending')->count();
                            @endphp
                            <x-nav-link :href="route('reports.index')" :active="request()->routeIs('reports.*')">
                                {{ __('Intern Reports') }}
                                @if($pendingReports > 0)
                                    <span class="ml-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white bg-red-500 rounded-full">{{ $pendingReports }}</span>
                                @endif
                            </x-nav-link>
                        @endif
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6 sm:space-x-3">
                ...
                <a href="{{ route('notifications.index') }}"
                   class="relative inline-flex items-center justify-center w-10 h-10 rounded-full text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-700 transition-colors duration-150"
                   title="Notifications">
                    ...
                    @if($unreadCount > 0)
                        <span class="absolute top-1 right-1 inline-flex items-center justify-center px-1.5 min-w-[1.15rem] h-[1.15rem] text-[10px] font-bold leading-none text-white bg-red-500 rounded-full ring-2 ring-white dark:ring-gray-800">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if(auth()->user()->isSuperAdmin() || auth()->user()->isAdmin())
                ...
                <x-responsive-nav-link :href="route('admin.departments')" :active="request()->routeIs('admin.departments')">{{ __('Departments') }}</x-responsive-nav-link>
            ...
            @else
                ...
                <x-responsive-nav-link :href="route('attendance.index')" :active="request()->routeIs('attendance.index')">{{ __('Attendance') }}</x-responsive-nav-link>
                @if(auth()->user()->department_id || auth()->user()->isSupervisor())
                <x-responsive-nav-link :href="route('attendance.team')" :active="request()->routeIs('attendance.team')">{{ __('Team Attendance') }}</x-responsive-nav-link>
                @endif
                ...
                @if(auth()->user()->isSupervisor())
                    @php
                        $pendingReportsM = \App\Models\Document::where('type', 'Internship Report')
                            ->whereIn('user_id', auth()->user()->subordinates()->where('is_intern', true)->pluck('id'))
                            ->where('status', 'pending')->count();
                    @endphp
                    <x-responsive-nav-link :href="route('reports.index')" :active="request()->routeIs('reports.*')">
                        {{ __('Intern Reports') }}@if($pendingReportsM > 0)<span class="ml-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white bg-red-500 rounded-full">{{ $pendingReportsM }}</span>@endif
